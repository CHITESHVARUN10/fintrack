...
function SuccessOverlay({ show, summary, onClose }: { show: boolean; summary?: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-on-surface/70 p-md">
      <div className="bg-white brutal max-w-md w-full p-xl flex flex-col items-center gap-md animate-[scaleIn_0.35s_ease]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-on-surface text-white">
                  <tr><th className="text-left px-sm py-xs">Date</th><th className="text-left px-sm py-xs">File</th><th className="text-left px-sm py-xs">Source</th><th className="text-right px-sm py-xs">Rows</th><th className="text-left px-sm py-xs">Result</th><th className="text-left px-sm py-xs">By</th></tr>
                </thead>
                <tbody>
                  {history.map((h) => (
                    <tr key={h.batchId} className="border-t-2 border-on-surface/20">
                      ...
                      <td className="px-sm py-xs truncate max-w-[220px]">{h.fileName || h.batchId.slice(0, 8)}</td>
                      <td className="px-sm py-xs">{h.source === 'BANK_STATEMENT' ? 'Bank' : 'Screenshot'}</td>
                      <td className="text-right px-sm py-xs">{h.total}</td>
                      ...
                      <td className="px-sm py-xs text-xs">{h.createdBy?.name || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
